...
def ComputeRBF_obj(CandPoint, data):
    ...
    if data.polynomial == 'none':
        ...
        PolyPart = data.ctail1 * np.ones((numpoints, 1))
    elif data.polynomial == 'linear':
        #print type(np.asmatrix(CandPoint))
        #print type(data.ctail1)
        PolyPart = np.concatenate((np.ones((numpoints, 1)), np.asmatrix(CandPoint)), axis = 1) * data.ctail1
    ...
    RBFvalue = np.asmatrix(U_Y).T * np.asmatrix(data.llambda1) + PolyPart
