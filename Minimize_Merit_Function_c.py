...
def Minimize_Merit_Function_c(data, CandPoint, NumberNewSamples,w_r):
    ...
    # initial linear regression surrogate surface for objective funtion MinPang
    parameter_poly = POLY(data.S[0:data.m, :],data.Y[0:data.m],'lin')
    #print 'CandPoint',CandPoint.shape, 'data.S[0:data.m, :]',data.S[0:data.m, :].shape
    NormValue = np.transpose(scp.distance.cdist(CandPoint, data.S[0:data.m, :]))
    CandValue = POLY_eval(CandPoint,parameter_poly,'lin')
    ...
    if NumberNewSamples == 1:
        ...
        #print 'ScaledCandValue',ScaledCandValue.shape,valueweight.shape,ScaledCandMinDist.shape
        ...
        for  ii in range(NumberNewSamples):
            ...
            if ii == 0: # select first candidate point
                ...
                #print type(xselected),xselected.shape
                ...
                # candidate point
                #print 'Minimized_merit xselected[ii-1, :]',xselected,CandPoint.shape[0]
                ...
                MinCandTotalValue = np.amin(CandTotalValue)
                selindex = np.argmin(CandTotalValue)
                #print 'xselected',np.array(xselected).shape, 'CandPoint[selindex, :]',np.asmatrix(CandPoint[selindex, :])
                xselected = np.concatenate((np.array(xselected), np.array(np.asmatrix(CandPoint[selindex, :]))), axis = 0)
                normval[ii] = np.asmatrix(NormValue[:, selindex])
