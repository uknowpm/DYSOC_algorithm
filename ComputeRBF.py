...
def ComputeRBF(CandPoint, data):
    ...
    if data.polynomial == 'none':
        ...
        #print type(np.asmatrix(CandPoint))
        #print type(data.ctail)
        PolyPart=[]
        for i in range(data.Cst_no):
            PolyPart.append(np.concatenate((np.ones((numpoints, 1)), np.asmatrix(CandPoint)), axis = 1) * data.ctail[i])
    ...
    for no_cst in range(data.Cst_no):
        if data.polynomial == 'linear':
            RBFvalue1 = np.asmatrix(U_Y).T * np.asmatrix(data.llambda[no_cst]) + PolyPart[no_cst]
        else:
            RBFvalue1 = np.asmatrix(U_Y).T * np.asmatrix(data.llambda[no_cst]) + PolyPart
        RBFvalue1 = np.array(RBFvalue1)
        RBFvalue1[RBFvalue1<0]=0 # put all negative prediction to zero Min
        
        if RBFvalue.shape[0]==0:
            RBFvalue=RBFvalue1
        else:
            RBFvalue=np.hstack((RBFvalue,RBFvalue1))
